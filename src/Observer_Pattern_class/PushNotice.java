...
public class PushNotice implements Observer {
    ...
    public void update(String userID, String action, int index) {
        if(!ID.equals(userID)) {
            String target = singleton.getItemProduct(index).getUserID();
            ...
            if (action.equals("판매")) {//접속자 대상이 아니라서 무조건 판매된것
                noticeObj = new NoticeObj(target, getComleteWordByJongsung(singleton.getItemProduct(index).getTitle(), "이", "가") + " 판매되었습니다.");
            } 
            else if (action.equals("매진")) {//자기가 자기것은 구매가 불가능하기 때문에 무조건 다른사람의 것을 구매한것
                noticeObj = new NoticeObj(target, getComleteWordByJongsung(singleton.getItemProduct(index).getTitle(), "이", "가") + " 모두 판매되었습니다.");
            } 
            else if (action.equals("삭제")) {//
                noticeObj = new NoticeObj(target, getComleteWordByJongsung(singleton.getItemProduct(index).getTitle(), "이", "가") + " 관리자에게 삭제되었습니다.");
            }
            else if (action.equals("수정")){
                noticeObj = new NoticeObj(target, getComleteWordByJongsung(singleton.getItemProduct(index).getTitle(), "이", "가") + " 관리자에게 수정되었습니다.");
            }
